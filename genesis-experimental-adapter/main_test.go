...
func TestACKIsNotVerified(t *testing.T) {
	k, _ := newKernel(1)
	if got := k.execute(publicEnvelope("1"), &probeRealizer{ack: true, observe: true}); got != observed {
		t.Fatalf("expected observed, got %s", got)
	}
}
...
func TestUnknownBlocksBlindRetry(t *testing.T) {
	...
	if got := k.execute(e, r); got != rejected {
		t.Fatalf("expected rejected blind retry, got %s", got)
	}
	r.mu.Lock()
	calls := r.calls
	r.mu.Unlock()
	if calls != 1 {
		t.Fatalf("realizer was called again: %d", calls)
	}
}
...
func TestUncertainRealizerErrorBlocksRetry(t *testing.T) {
	...
	if got := k.execute(e, r); got != rejected {
		t.Fatalf("expected rejected retry, got %s", got)
	}
}

func TestObservationIdentityFailsClosed(t *testing.T) {
	k, _ := newKernel(1)
	if got := k.execute(publicEnvelope("1"), &probeRealizer{ack: true, observe: true, badID: true}); got != unknown {
		t.Fatalf("expected unknown, got %s", got)
	}
}

func TestMissingObservationDigestFailsClosed(t *testing.T) {
	k, _ := newKernel(1)
	if got := k.execute(publicEnvelope("1"), &probeRealizer{ack: true, observe: true, missingDigest: true}); got != unknown {
		t.Fatalf("expected unknown, got %s", got)
	}
}
...
func TestHITLBoundary(t *testing.T) {
	...
	if got := k.execute(e, &probeRealizer{ack: true, observe: true}); got != observed {
		t.Fatalf("expected observed, got %s", got)
	}
}
...
func TestBoundedConcurrentAdmission(t *testing.T) {
	...
	wg.Wait()
	if accepted != 1 {
		t.Fatalf("expected exactly one admission, got %d", accepted)
	}
}
