...
func (k *kernel) admit(e envelope) error {
	...
	for _, c := range e.Constraints {
		switch c.Key {
		case "target":
			if c.Value != e.Transition.Target { return errors.New("constraint target mismatch") }
		case "operation":
			if c.Value != e.Transition.Operation { return errors.New("constraint operation mismatch") }
		default:
...
func (k *kernel) execute(e envelope, r realizer) outcome {
	...
	if !a.Ack || a.Observation == nil {
		k.mu.Lock(); k.status[e.Transition.ID] = unknown; k.mu.Unlock()
		return unknown
	}
	k.mu.Lock(); k.status[e.Transition.ID] = observed; k.mu.Unlock()
	return observed
}

func (k *kernel) reconcile(e envelope, r realizer) outcome {
	k.mu.Lock(); s := k.status[e.Transition.ID]; k.mu.Unlock()
	if s != unknown { return rejected }
	a := r.Reconcile(e.Transition)
	if a.Observation == nil { return unknown }
	k.mu.Lock(); k.status[e.Transition.ID] = observed; k.mu.Unlock()
	return observed
}

func has(values []string, wanted string) bool {
	for _, v := range values { if v == wanted { return true } }
	return false
}

type probeRealizer struct { ack, observe bool; calls int; mu sync.Mutex }
...
func (r *probeRealizer) Reconcile(t transition) attempt { return attempt{Observation: &observation{TransitionID: t.ID}} }
...
func publicEnvelope(id string) envelope {
	return envelope{
		...
		Constraints: []constraint{{Key: "target", Value: "probe-target"}, {Key: "operation", Value: "probe"}},
	}
}
