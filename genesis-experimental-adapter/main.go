...
type kernel struct { mu sync.Mutex; capacity int; active map[string]struct{}; status map[string]outcome }
...
func (k *kernel) admit(e envelope) error {
	...
	for _, c := range e.Constraints {
		switch c.Key {
		case "target": if c.Value != e.Transition.Target { return errors.New("constraint target mismatch") }
		case "operation": if c.Value != e.Transition.Operation { return errors.New("constraint operation mismatch") }
		case "payload-digest": if c.Value != digest(e.Transition.Payload) { return errors.New("constraint payload mismatch") }
		default: return errors.New("unknown constraint")
		}
	}
	...
	k.active[e.Transition.ID] = struct{}{}; k.status[e.Transition.ID] = "ATTEMPTED"
	return nil
}
...
func (k *kernel) execute(e envelope, r realizer) outcome {
	...
	if a.Err != nil || !a.Ack || a.Observation == nil {
		k.mu.Lock(); k.status[e.Transition.ID] = unknown; k.mu.Unlock(); return unknown
	}
...
func (k *kernel) reconcile(e envelope, r realizer) outcome {
	k.mu.Lock(); s := k.status[e.Transition.ID]; k.mu.Unlock()
	if s != unknown || strings.TrimSpace(e.Transition.ID) == "" { return rejected }
	for _, c := range e.Constraints {
		switch c.Key { case "target": if c.Value != e.Transition.Target { return rejected }; case "operation": if c.Value != e.Transition.Operation { return rejected }; case "payload-digest": if c.Value != digest(e.Transition.Payload) { return rejected }; default: return rejected }
	}
...
func publicEnvelope(id string) envelope { return envelope{Transition: transition{ID: id, Target: "probe-target", Operation: "probe", Payload: "public-fixture"}, Capability: capability{Operations: []string{"probe"}, Targets: []string{"probe-target"}}, Authority: authority{Operations: []string{"probe"}, Targets: []string{"probe-target"}}, Constraints: []constraint{{Key: "target", Value: "probe-target"}, {Key: "operation", Value: "probe"}, {Key: "payload-digest", Value: digest("public-fixture")}}} }

func main() {
	k, err := newKernel(2); if err != nil { panic(err) }
	if got := k.execute(publicEnvelope("public-1"), &probeRealizer{ack: true, observe: true}); got != observed { panic(fmt.Sprintf("expected observed, got %s", got)) }
