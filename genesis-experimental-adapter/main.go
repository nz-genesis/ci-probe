...
type capability struct {
	ID                  string
	Operations, Targets []string
}
type authority struct {
	Principal                  string
	Operations, Targets        []string
	RequiresHITL, ApprovedHITL bool
}
...
func (k *kernel) admit(e envelope) error {
	if strings.TrimSpace(e.Transition.ID) == "" || strings.TrimSpace(e.Transition.Target) == "" || strings.TrimSpace(e.Transition.Operation) == "" {
	...
	if err := validateConstraints(e.Transition, e.Constraints); err != nil {
		return err
	}
	...
	k.status[e.Transition.ID] = "ATTEMPTED"
	k.admissions[e.Transition.ID] = fp
	return nil
}
func (k *kernel) execute(e envelope, r realizer) outcome {
	...
	k.mu.Unlock()
	if a.Err != nil || !a.Ack || a.Observation == nil {
		k.mu.Lock()
		k.status[e.Transition.ID] = unknown
		k.mu.Unlock()
		return unknown
	}
	if a.Observation.TransitionID != e.Transition.ID || strings.TrimSpace(a.Observation.Digest) == "" {
		k.mu.Lock()
		k.status[e.Transition.ID] = unknown
		k.mu.Unlock()
		return unknown
...
func (k *kernel) reconcile(e envelope, r realizer) outcome {
	...
	expected := k.admissions[e.Transition.ID]
	k.mu.Unlock()
	if s != unknown || strings.TrimSpace(e.Transition.ID) == "" {
		return rejected
	}
	if err := validateConstraints(e.Transition, e.Constraints); err != nil {
		return rejected
	}
	if expected == "" || envelopeFingerprint(e) != expected {
		return rejected
	}
	a := r.Reconcile(e.Transition)
	if a.Observation == nil || a.Observation.TransitionID != e.Transition.ID || strings.TrimSpace(a.Observation.Digest) == "" {
		return unknown
	}
	k.mu.Lock()
	k.status[e.Transition.ID] = observed
...
func validateConstraints(t transition, cs []constraint) error {
	for _, c := range cs {
		...
		case "operation":
			if c.Value != t.Operation {
				return errors.New("constraint operation mismatch")
			}
		case "payload-digest":
			if c.Value != digest(t.Payload) {
				return errors.New("constraint payload mismatch")
			}
		default:
...
func envelopeFingerprint(e envelope) string {
	ops := append([]string(nil), e.Capability.Operations...)
	targets := append([]string(nil), e.Capability.Targets...)
	authOps := append([]string(nil), e.Authority.Operations...)
	authTargets := append([]string(nil), e.Authority.Targets...)
	cs := append([]constraint(nil), e.Constraints...)
	sort.Strings(ops)
	sort.Strings(targets)
	sort.Strings(authOps)
	sort.Strings(authTargets)
...
type probeRealizer struct {
	ack, observe, err, badID, missingDigest bool
	calls                                   int
	mu                                      sync.Mutex
}
func (r *probeRealizer) Execute(t transition) attempt {
	...
	r.calls++
	if r.err {
		return attempt{Ack: true, Err: errors.New("uncertain realizer error")}
	}
	if !r.observe {
		return attempt{Ack: r.ack}
	}
	id, d := t.ID, digest(t.Payload)
	if r.badID {
		id = "other"
	}
	if r.missingDigest {
