...
A PASS here would establish only that the generic commitment-envelope mechanism has the desired bounded correspondence properties. It would **not** establish that a real Genesis semantic vector can be safely projected, because the fixture is not private Genesis state and the cryptographic construction is a test mechanism rather than a Genesis primitive.
...
`BOUNDED / EXPERIMENTAL / HOSTED-CI PENDING / NOT CANONICAL`
