...
def main() -> None:
    ...
    # 2. Boundary replacement is a distinct operation and is not admitted by this self-change gate.
    ...
    # 4. Candidate dependency/root cannot redefine current authority.
    ...
    # 7. Weak verifier cannot rewrite protected qualification semantics.
    assert not qualify(state, boundary, replace(verifier, weak=True), deps, t)
    # 8. Trust-root substitution is rejected even if verifier name/version match.
    assert not qualify(state, boundary, replace(verifier, trusted_root="ROOT-ATTACKER"), deps, t)
    # 9. Boundary guard cannot be disabled by mutable candidate semantics.
    assert not qualify(state, replace(boundary, constitutional_guard=False), verifier, deps, t)
    ...
    # 13. A self-authored candidate state changing the boundary is not current.
    ...
    # 14. An unrelated boundary identity cannot be substituted silently.
