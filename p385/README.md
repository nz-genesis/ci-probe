...
The public harness contains no private Genesis source, private Genesis research corpus, private prompts, private fixtures, credentials, or canonical decisions.
...
A queued or configured workflow is not execution evidence. Only a completed job with an assigned runner, observable steps/logs, and a reproducible receipt counts as external runtime evidence.
...
The current first gate is external implementation execution and its native test suite. Subsequent concordance work may add only public, source-native challenge fixtures whose semantics are frozen before any Genesis projection.
