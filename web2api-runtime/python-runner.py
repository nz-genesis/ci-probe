...
try:
    with sync_playwright() as p:
        t = time.perf_counter(); context = p.chromium.launch_persistent_context(str(profile_a), headless=True); result["timings_ms"]["first_launch"] = (time.perf_counter()-t)*1000
        page = context.new_page(); page.goto(base); page.get_by_role("button", name="Run request").click(); page.locator("#state").wait_for(state="visible")
        result["checks"]["semantic_interaction"] = page.locator("#state").text_content() == "1"
        result["checks"]["request_correlation"] = bool(page.evaluate("localStorage.getItem('request_id')")); context.close()
        t = time.perf_counter(); context = p.chromium.launch_persistent_context(str(profile_a), headless=True); result["timings_ms"]["restart"] = (time.perf_counter()-t)*1000
        page = context.new_page(); page.goto(base); result["checks"]["persistence"] = bool(page.evaluate("localStorage.getItem('request_id')")); context.close()
        context = p.chromium.launch_persistent_context(str(profile_b), headless=True); page = context.new_page(); page.goto(base)
        result["checks"]["profile_isolation"] = not bool(page.evaluate("localStorage.getItem('request_id')")); context.close()
    result["checks"]["all_pass"] = all(result["checks"].values())
    print(json.dumps(result, indent=2))
    if not result["checks"]["all_pass"]: raise SystemExit(1)
finally:
