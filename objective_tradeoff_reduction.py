...
Objective data is State; admissibility is Constraint + Capability + Authority;
selection is a Transition-level choice. No Genesis runtime is used.
"""
...
def admissible(candidate, state):
    if candidate["capability"] not in state["capabilities"]: return False
    if candidate["authority"] not in state["authorities"]: return False
    if candidate["cost"] > state["constraints"]["max_cost"]: return False
    if candidate["risk"] > state["constraints"]["max_risk"]: return False
    return True


def score(candidate, state):
    return tuple(candidate["values"].get(name, 0) * weight for name, weight in state["objectives"])
...
def select(candidates, state):
    ...
    if len(ranked) > 1 and score(ranked[0], state) == score(ranked[1], state): return "AMBIGUOUS"
    return ranked[0]["id"]
...
CANDIDATES = [
    ...
    {"id": "tie", "capability": "plan", "authority": "policy", "cost": 2, "risk": 2, "values": {"safety": 5, "speed": 5}},
]
BASE = {"objectives": [("safety", 2), ("speed", 1)], "capabilities": {"plan"}, "authorities": {"policy"}, "constraints": {"max_cost": 10, "max_risk": 10}}
